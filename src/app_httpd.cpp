...
void setupLedFlash(int pin) {
#if CONFIG_LED_ILLUMINATOR_ENABLED
    int ledChannel = 0; // Choose a PWM channel (0-15)
    int freq = 5000; // Frequency in Hz
    int resolution = 8; // PWM resolution (8-16 bits)
    ledcSetup(ledChannel, freq, resolution); // Configure the PWM channel
    ledcAttachPin(pin, ledChannel); // Attach the pin to the PWM channel
#else
    log_i("LED flash is disabled -> CONFIG_LED_ILLUMINATOR_ENABLED = 0");
#endif
}
